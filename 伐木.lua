...
game:GetService("Workspace").Stores.WoodRUs.Parts.WOODDROPOFF.SurfaceGui.TextLabel.Text = "刺客脚本";
...
local function donate(plr, amount)
    ...
    notify('刺客', '转钱' .. tostring(amount) .. ' 给 ' .. tostring(plr), 4);
    scr2.exitNotice();
end
...
local Cloudlib = loadstring(game:HttpGet"https://raw.githubusercontent.com/SXQi8888/CS78/main/CS.txt")()
local window = Cloudlib:new("伐木大亨", '')
local creds = window:Tab("关于脚本", "4370344717")
local Q = creds:section("信息",true)
local Tab = window:Tab("主要功能", "4370345144")
local Tab1 = window:Tab("调节功能", "3605022185")
local Section3 = Tab1:section("人物调节",true)
local V = window:Tab("传送功能", "3944688398")
local Section4 = V:section("普通传送",true)
local Sectiontptree = V:section("传送到树旁边",true)
local Section6 = V:section("汽车传送",true)
local Sectionchuanson = V:section("传送物品",true)
local Section = Tab:section("斧头",true)
local A = window:Tab("基地功能", "4370305588")
local Section1 = A:section("基地",true)
local B = window:Tab("树木功能", "4370343755")
local Section5 = B:section("木头",true)
local Sectionbringtree = B:section("带来树",true)
local C = window:Tab("换钱功能", "3610253853")
local Sectionjinbi = C:section("快速转钱", true)
local D = window:Tab("汽车功能", "4506892784")
local Sectionqiche = D:section("汽车",true)
local Sectionfengche = D:section("刷粉车", true)
local Sectionqita = Tab:section("其他功能",true)
local Sectionshuoh = Tab:section("刷屏",true)
local Sectionyule = Tab:section("娱乐区域",true)
local Sectionzhenglimt = Tab:section("整理木板",true)
local Sectionzhengli = Tab:section("整理",true)
local Sectiontchon = Tab:section("填充蓝图（用木头）",true)
local Tab2 = window:Tab("环境功能", "4483362458")
local Sectionhuanjin = Tab2:section("环境",true)
local Tab4 = window:Tab("猎魔功能", "4370317406")
local Sectionmogui = Tab4:section("猎魔功能",true)
local Tab5 = window:Tab("自动购买", "3944664684")
local Sectionautobuy = Tab5:section("自动购买",true)
local Sectionautobuy6 = Tab5:section("其他",true)

Q:Label("你的用户名："..game.Players.LocalPlayer.Name)
Q:Label("你的注入器:"..identifyexecutor())
Q:Label("您当前服务器的ID"..game.GameId)
Q:Button(">欢迎使用刺客脚本<", function()
end)
Q:Button("作者:SheFa.", function()
end)
Q:Button("官方Q群637340150", function()
end)
...
Section:Button("获取当前死亡后加载时间", function()
    bailib:Notification("刺客", " 时间为" .. bai.loaddupeaxewaittime .. "秒", "好")
end)
...
Section3:Button("队伍透视", function()
    loadstring(game:HttpGet("https://raw.github.com/SXQi8888/WCNM/main/ESP.Pro"))()
end)
     
Section3:Toggle("人物无限跳", "", false, function(Value)
    Jump = Value
...
Section3:Toggle("人物穿墙", "", false, function(Value)
    Noclip = Value
...
Sectionhuanjin:Button("船离开时间", function()
    ...
    if game.Workspace.Ferry.TimeToDeparture.Value == 0 then
        ...
        notify("刺客", "距离船离开时间有" .. game.Workspace.Ferry.TimeToDeparture.Value .. "秒", 4)
    end
end)
Sectionhuanjin:Button("桥上升时间", function()
    ...
    if game.Workspace.Bridge.DownTime.Value < 0 then
        ...
        notify("刺客", "距离桥上升的时间有" .. game.Workspace.Bridge.DownTime.Value .. "秒", 4)
    end
end)
Sectionhuanjin:Button("查看游戏时间", function()
    notify("刺客", "游戏时间为" .. tostring(game.Lighting.TimeOfDay):sub(1, -2):sub(1, -2):sub(1, -2), 4)
end)
